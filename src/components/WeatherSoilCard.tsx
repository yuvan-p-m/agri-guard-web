...
import type { Language, WeatherInfo, UserProfile } from '../types';
...
export const WeatherSoilCard: React.FC<WeatherSoilCardProps> = ({
  ...
  const handleSendTestSms = async () => {
    ...
    setSmsResult(null);
    try {
      const phoneToSend = user?.phone || prompt('Enter 10-digit mobile number to receive SMS:');
      if (!phoneToSend) {
      ...
      if (res?.success) {
        setSmsResult('✅ SMS advisory sent successfully to your mobile via Fast2SMS!');
      } else {
        setSmsResult(`⚠️ Fast2SMS notice: ${res?.notice || res?.warning || 'Dispatched'}`);
      }
    } catch (err: any) {
      setSmsResult(`⚠️ Failed to send SMS: ${err?.message || 'Gateway error'}`);
    } finally {
      setIsSendingSms(false);
    }
  };
    ...
    <div className="bg-white/95 backdrop-blur-md rounded-3xl p-5 sm:p-6 shadow-xl border border-agri-200/80 relative">
        ...
        <div className="flex items-center gap-2">
          {onRefreshLocation && (
            <button
              ...
              disabled={isLoading}
              title={t('refreshBtn', 'Refresh Live GPS Location')}
            ...
            >
              ...
              <span className="hidden sm:inline">{t('refreshBtn', 'Refresh')}</span>
            </button>
          )}
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-extrabold ${
            ...
            <span>{isGpsDenied ? t('gpsDenied', 'GPS Denied (Manual)') : t('liveGpsActive', '📍 Live GPS Active')}</span>
          </div>
        </div>
      </div>
      ...
      {isGpsDenied && (
        <form onSubmit={handleManualSubmit} className="mt-3 p-3 rounded-2xl bg-amber-50/80 border border-amber-200/80">
          <div className="flex items-center gap-1.5 text-xs font-bold text-amber-900 mb-2">
            ...
            <span>{t('gpsDeniedTitle', 'GPS Location Permission Denied / Unavailable')}</span>
          </div>
          <p className="text-[11px] text-amber-800 mb-2">
            {t('gpsDeniedDesc', 'Since GPS access was denied, enter your farm city name manually to fetch live OpenWeatherMap forecast:')}
          </p>
          <div className="flex gap-2">
            ...
            >
              ...
              <span>{t('searchCityBtn', 'Search')}</span>
            </button>
          </div>
        </form>
      )}
      ...
      {isLoading ? (
        <div className="py-12 text-center space-y-3">
          ...
          <p className="text-xs font-bold text-slate-600">{t('fetchingTelemetry', 'Fetching live meteorological telemetry...')}</p>
        </div>
      ) : (
        <>
          ...
          <div className="flex items-center justify-between mt-4 p-4 rounded-2xl bg-gradient-to-br from-agri-50 via-emerald-50/50 to-blue-50/50 border border-agri-200/80">
            <div>
              ...
              <p className="text-[11px] text-slate-600">
                {weather.city.toLowerCase().includes('fetching')
                  ? t('fetchingLocation', 'Fetching location...')
            ...
            <div className="text-right">
              <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-extrabold ${
                ...
                {t(`risk${weather.fungalRiskLevel.replace(/\s+/g, '')}`, `${weather.fungalRiskLevel} Risk`)} {t('fungalRiskLabel', 'Fungal Risk')}
              </span>
              <p className="text-[11px] text-slate-500 mt-1.5 flex items-center justify-end gap-1 font-medium">
                <Clock className="w-3 h-3 text-slate-400" />
                {weather.leafWetnessHours}h {t('leafWetnessLabel', 'Leaf Wetness')}
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="text-left w-full sm:w-auto">
              <span className="text-xs font-extrabold text-slate-900 block flex items-center gap-1.5">
                ...
                <span>Fast2SMS Live Weather & Irrigation Advisory</span>
              </span>
              <span className="text-[11px] text-slate-500 font-medium">
                Sends live temperature, humidity, rain status & irrigation time to {user?.phone || 'registered phone'}
              </span>
            </div>
            <button
              onClick={handleSendTestSms}
              disabled={isSendingSms}
              className="w-full sm:w-auto px-4 py-2 rounded-xl bg-agri-800 hover:bg-agri-900 text-white text-xs font-bold shadow-md shadow-agri-950/20 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50 shrink-0"
            >
              <Smartphone className="w-3.5 h-3.5 text-citrus-300" />
              <span>{isSendingSms ? 'Dispatching SMS...' : 'Send Live SMS to Phone'}</span>
            </button>
          </div>
          {smsResult && (
            <p className="mt-2 text-[11px] font-bold text-agri-950 p-2 rounded-xl bg-agri-50 border border-agri-200 animate-fade-in">{smsResult}</p>
          )}
        </>
      )}
